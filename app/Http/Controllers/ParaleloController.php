    ...
    public function storeParalelo(Request $request)
{
    try {
        ...
        // Validar que no haya otros paralelos activos con el mismo nombre
        $paraleloDuplicado = ParaleloModel::where('paralelo', $request->paralelo)
            ->where('estado', 'A')
            ->first();

        if ($paraleloDuplicado) {
            return response()->json([
                "ok" => false,
                "message" => "El paralelo ya existe y está activo"
            ], 400);
        }
    ...
    public function updateParalelo(Request $request, $id)
{
    try {
        ...
        // Verificar si ya existe otro paralelo con el mismo nombre y que esté activo
        $paraleloExistente = ParaleloModel::where('paralelo', $request->paralelo)
            ->where('estado', 'A') // Solo buscamos paralelos activos
            ->where('id_paralelo', '!=', $id) // Asegurarnos de que no se compare con el mismo registro
            ->first();
        ...
        // Actualizar los campos del paralelo con los datos proporcionados
        $paralelo->update([
